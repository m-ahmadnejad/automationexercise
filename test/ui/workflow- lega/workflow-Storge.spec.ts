...
import { addToCart, addToCartAndPlaceOrder, addToCartAndProceedToCheckout } from '../../../utils/commonHelper'
...
test('add item to cart', async({page,productPage})=>{
  await productPage.AddToCart('Blue Top')
  await expect(page.locator('.modal-content')).toBeVisible()
  await expect(page.getByText('Added!')).toBeVisible()
  })
test('view cart', async({page,productPage,viewCartPage})=>{
...
test('verify remove item from view cart',async({page,productPage,viewCartPage})=>{
  await addToCart(productPage,'Blue Top')
  const item = await viewCartPage.viewCart('Blue Top')
  console.log('item name ******* :',item.name,'item price *******:',item.price)
...
test('checkout page review order ',async({page,productPage,viewCartPage,checkoutPage})=>{
...
test('ubmitting payment redirects to payment done page', async({page,productPage,viewCartPage,checkoutPage,paymentPage})=>{
...
test('payment done page shows success confirmation', async({page,productPage,checkoutPage,viewCartPage,paymentDonePage,paymentPage})=>{
  await addToCartAndPlaceOrder(productPage,'Blue Top',viewCartPage,checkoutPage)
